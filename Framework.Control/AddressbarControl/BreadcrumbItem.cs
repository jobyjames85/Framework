...
namespace Framework.Controls
{
    ...
    public class BreadcrumbItem : Selector
    {
        ...
        public static readonly DependencyProperty PageUriProperty =
            DependencyProperty.Register("PageUri", typeof(string), typeof(BreadcrumbItem), new PropertyMetadata(string.Empty));
        ...
        private const string PropertyValueNotSet = "Value_Not_Set";
        BreadcrumbItem withChildBreadcrumbItem;
        BreadcrumbItem withoutChildBreadcrumbItem;
        Dictionary<int, BreadcrumbItem> DicBreadCrumbItemAddressBar = new Dictionary<int, BreadcrumbItem>();
        [property: CLSCompliant(false)]
        public List<SystemMenuTreeList> ItemSource
        {
            get { return (List<SystemMenuTreeList>)GetValue(ItemSourceProperty); }
            set { SetValue(ItemSourceProperty, value); }
        }

        // Using a DependencyProperty as the backing store for ItemSource.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty ItemSourceProperty =
            DependencyProperty.Register("ItemSource", typeof(List<SystemMenuTreeList>), typeof(BreadcrumbItem), new PropertyMetadata(null));

        /// <summary>
        /// 
        /// </summary>
        public void GenerateBreadcrumTree()
        {
            if (ItemSource != null)
            {
                var getParent = ItemSource.Where(p => p.PId == 0).OrderBy(p => p.DisplayIndex);
                if (DicBreadCrumbItemAddressBar.Any())
                {
                    DicBreadCrumbItemAddressBar.Clear();
                }
                GenerateBar(getParent);
            }
        }

        [CLSCompliant(false)]
        public void generateBread(List<SystemMenuTreeList> items)
        {
            ItemSource = items;
            GenerateBreadcrumTree();
        }
        /// <summary>
